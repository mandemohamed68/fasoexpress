    ...
    if (!inString) {
      ...
      if (isEscaped) {
        ...
        if (char === '\\') {
          ...
          // ignore carriage returns inside string
        } else if (char === '\t') {
          result += '\\t';
        } else {
          result += char;
...
function sanitizePrivateKey(rawKey: string): string {
  ...
  if (headerMatch && footerMatch) {
    ...
    let cleanBody = pk.replace(/-----BEGIN [A-Z ]+-----/g, '')
                      .replace(/-----END [A-Z ]+-----/g, '')
                      .replace(/[\s\\"']/g, '');
...
function getFirebaseAdmin() {
  ...
  if (serviceAccountVar) {
    try {
      ...
      if (serviceAccount) {
        ...
        console.log("[FCM] Firebase Admin initialisé avec succès !");
  ...
  const saPaths = [
    ...
    path.join(process.cwd(), "fasoexpress-2f11e-firebase-adminsdk-fbsvc-f7f6a6391b.json")
  ];
  ...
  for (const saPath of saPaths) {
    if (fs.existsSync(saPath)) {
      try {
        ...
        console.log(`[FCM] Firebase Admin initialisé avec le fichier ${path.basename(saPath)}.`);
        return firebaseAdminApp;
      } catch (e) {
        console.error(`[FCM] Échec d'initialisation de Firebase Admin avec ${path.basename(saPath)}:`, e);
      }
    }
  }
...
async function startServer() {
  ...
  const paymentLimiter = rateLimit({
    windowMs: 15 * 60 * 1000,
    max: 50,
    message: { error: "Trop de requêtes de paiement. Veuillez patienter." },
    standardHeaders: true,
    legacyHeaders: false,
    validate: { trustProxy: false },
  });
...
const MASTER_ADMIN_EMAILS = ['mandemohamed68@gmail.com', 'mandemohamed6868@gmail.com'];
  ...
  app.post("/api/auth/register", async (req, res) => {
    ...
    try {
      ...
      const token = jwt.sign({ userId, email, role: targetRole }, JWT_SECRET, { expiresIn: '7d' });
  ...
  app.post("/api/auth/login", async (req, res) => {
    ...
    try {
      ...
      const token = jwt.sign({ userId: user.userId || user.id, email: user.email, role: user.role }, JWT_SECRET, { expiresIn: '7d' });
      res.json({ token, user });
    } catch (error) {
  ...
  app.post("/api/auth/forgot-password", async (req, res) => {
    ...
    try {
      ...
      if (host && userMail && passMail) {
        ...
        const cfgRow = db.prepare("SELECT value FROM config WHERE `key` = 'app_config'").get() as any;
        const cfgObj = cfgRow && cfgRow.value ? JSON.parse(cfgRow.value) : {};
        const footerPublisher = (cfgObj.companyNameActive !== false && (cfgObj.companyName || 'SAPPAY TECHNOLOGIE')) 
          ? `<p style="color: #94a3b8; font-size: 11px; text-align: center; margin-bottom: 0;">Édité par ${cfgObj.companyName || 'SAPPAY TECHNOLOGIE'}</p>`
          : '';
        ...
        return res.json({ 
          status: "ok", 
          message: "Un code de réinitialisation a été généré et envoyé si l'adresse est valide." 
        });
      }
  ...
  app.patch("/api/profile", authenticate, async (req: any, res) => {
    ...
    // Prevent non-admin users from escalating privileges or altering sensitive balance/status fields
    const PROTECTED_FIELDS = new Set([
      'role', 'accountStatus', 'isVerified', 'verificationStatus', 'balance', 'earnings', 'resetCode', 'resetExpires', 'totalWithdrawn'
    ]);
  ...
  app.post("/api/deliveries", authenticate, (req: any, res) => {
    ...
    try {
      ...
      let clientProposedPrice = Number(d.clientProposedPrice || d.cost || calculatedCost || 1000);
      let baseCost = Number(d.baseCost || d.estimatedCost || calculatedCost || 1000);
  ...
  app.post("/api/db-query-tool", authenticate, checkAdmin, (req: any, res) => {
    return res.status(403).json({ success: false, error: "L'outil d'exécution SQL direct est désactivé en production pour des raisons de sécurité." });
  });
  ...
  const normalizePhoneNumberSappay = (phone: string, processorId?: string) => {
    ...
    // Désormais, pour TOUS les opérateurs (Moov Money, Coris Money, Orange Money, Telecel Money),
    // la passerelle Sappay s'attend à un numéro national à 8 chiffres sans l'indicatif 226 au début.
  ...
  async function getSappayToken() {
    ...
    if (!clientId || !clientSecret || !username || !password) {
      try {
        ...
        if (!clientId || !clientSecret || !username || !password) {
          try {
            const sapPayRow = db.prepare("SELECT * FROM config_store WHERE id = 'sappay'").get() as any;
            ...
            // Ignore if config_store is empty or not initialized
          }
        }
      } catch (dbErr: any) {
    ...
    if (!clientId || !clientSecret || !username || !password) {
      console.warn("[SAPPAY] Identifiants de paiement non configurés. Activation du mode SANDBOX de test pour le paiement.");
  ...
  app.get("/api/payment/sappay/config-check", authenticate, checkAdmin, async (req, res) => {
    try {
      ...
      if (token === "MOCK_TOKEN_SANDBOX_12345") {
        return res.json({
          status: "sandbox",
          message: "SAPPAY est actuellement en MODE DEMO (Sandbox). Veuillez renseigner de vrais identifiants de production pour passer en mode réel."
        });
      }
      res.json({ 
        status: "success", 
  ...
  app.post("/api/payment/sappay/get-otp", async (req, res) => {
    try {
      ...
      // Pause de 800ms pour laisser la facture s'indexer chez Sappay
      ...
      // Si Sappay renvoie "Paramètres erronés !" dû à un délai d'indexation très rare de la facture, on retente une unique fois 1.2s après
      const respMsg = (data?.response?.message || data?.message || "").toString().toLowerCase();
      if (data && (respMsg.includes("paramètres erronés") || respMsg.includes("parametres errones"))) {
        console.warn("[Sappay OTP] Temporary indexing error 'Paramètres erronés !', retrying once after 1200ms...");
  ...
  app.post("/api/payment/sappay/perform", async (req, res) => {
    try {
      ...
      if (!response.ok) {
        ...
        // Analyse si le corps de réponse contient en réalité un message de succès de l'opérateur (ex: Orange, Moov, Telecel, Coris)
        const lowerResp = responseText.toLowerCase();
        const hasSuccessText = 
          lowerResp.includes("successfully") || 
          lowerResp.includes("completed") || 
          lowerResp.includes("transaction of fcfa") || 
          lowerResp.includes("reussie") || 
          lowerResp.includes("réussie") || 
          lowerResp.includes("succes") || 
          lowerResp.includes("succès") || 
          lowerResp.includes("effectué") || 
          lowerResp.includes("effectue") || 
          lowerResp.includes("approuvé") ||
          lowerResp.includes("approuve") ||
          lowerResp.includes("approved");

        const hasExplicitErrorText = 
          lowerResp.includes("failed") || 
          lowerResp.includes("echec") || 
          lowerResp.includes("échec") || 
          lowerResp.includes("incorrect") || 
          lowerResp.includes("invalid") || 
          lowerResp.includes("insuffisant") || 
          lowerResp.includes("refused") || 
          lowerResp.includes("declined") || 
          lowerResp.includes("annul") ||
          lowerResp.includes("otp does not exist") ||
          lowerResp.includes("does not exist") ||
          lowerResp.includes("not found");
        ...
        if (hasSuccessText && !hasExplicitErrorText) {
          ...
          let parsedData: any = {};
          try { parsedData = JSON.parse(responseText); } catch (_) { parsedData = { message: responseText }; }
  ...
  // --- AUTOMATIC TABLE PARTITIONING ENDPOINTS ---
  app.get("/api/admin/system/partitions-status", authenticate, checkAdmin, (req: any, res) => {
    try {
      ...
      let dbConfigSaved: any = null;
      try {
        const row = db.prepare("SELECT value FROM config WHERE `key` = 'db_partition_status'").get() as any;
        if (row && row.value) {
      ...
      tablesList.forEach((tbl) => {
        ...
        try {
          const row = db.prepare(`SELECT COUNT(*) as count FROM ${tbl}`).get() as any;
          count = row?.count || 0;
        } catch (_) {
          tableExists = false;
        }
        ...
        let archiveCount = 0;
        try {
          const archRow = db.prepare(`SELECT COUNT(*) as count FROM ${tbl}_partition_archive`).get() as any;
          archiveCount = archRow?.count || 0;
        } catch (_) {}

        let partitionStatus = 'ok';
        let healthColor = 'emerald'; // emerald = 🟢, amber = 🟠, rose = 🔴
        let statusText = 'Partitionnée & Indexée';

        if (!tableExists) {
          partitionStatus = 'missing';
          healthColor = 'amber';
          statusText = 'Non créée';
        } else if (isFailedOverall) {
          partitionStatus = 'error';
          healthColor = 'rose';
          statusText = 'Échec Partitionnement';
        } else if (count > 5000 && !dbConfigSaved?.lastRun) {
          partitionStatus = 'warning';
          healthColor = 'amber';
          statusText = 'Partitionnement Recommandé';
        } else {
          partitionStatus = 'ok';
          healthColor = 'emerald';
        ...
        tableStats.push({
          ...
          healthColor,
          statusText,
          partitionType: ['tracking', 'notifications', 'messages'].includes(tbl) ? 'Par Plage Temporelle & Archive' : 'Par Clé Composite & Indexation'
        });
      });
      ...
      console.error("Erreur récupération statut des partitions:", err);
      res.status(500).json({ error: "Échec de récupération du statut des partitions", details: err?.message });
    }
  });

  app.post("/api/admin/system/auto-partition", authenticate, checkAdmin, (req: any, res) => {
    ...
    try {
      ...
      logDetails.push(`[${new Date().toLocaleTimeString()}] Analyse pré-partitionnement de la base de données...`);
      logDetails.push(`Rétention demandée : ${retentionDays} jours.`);
      
      // 1. Diagnostic de santé DB
      if (!db.engine || db.engine.includes('SQLite')) {
        ...
        if (integrity && integrity.integrity_check !== 'ok' && integrity['integrity_check'] !== 'ok') {
          throw new Error(`Innocuité de la base compromise: ${JSON.stringify(integrity)}`);
        }
        logDetails.push("Check d'intégrité de la base de données : OK 🟢");
      }

      // 2. Création des structures de partition d'archivage si absentes
      db.exec(`
      ...
      `);
      logDetails.push("Tables de partitionnement secondaires créées/vérifiées.");

      // 3. Création des Index de Partitionnement haute performance
      tablesToPartition.forEach((tbl) => {
        try {
          if (tbl === 'deliveries') {
            db.exec("CREATE INDEX IF NOT EXISTS idx_deliveries_part_created ON deliveries(createdAt, status)");
            db.exec("CREATE INDEX IF NOT EXISTS idx_deliveries_part_client ON deliveries(clientId, status)");
            db.exec("CREATE INDEX IF NOT EXISTS idx_deliveries_part_driver ON deliveries(driverId, status)");
          } else if (tbl === 'tracking') {
            db.exec("CREATE INDEX IF NOT EXISTS idx_tracking_part_del ON tracking(deliveryId, timestamp)");
          } else if (tbl === 'messages') {
            db.exec("CREATE INDEX IF NOT EXISTS idx_messages_part_del ON messages(deliveryId, createdAt)");
          } else if (tbl === 'notifications') {
            db.exec("CREATE INDEX IF NOT EXISTS idx_notifications_part_user ON notifications(userId, createdAt)");
          } else if (tbl === 'users') {
            db.exec("CREATE INDEX IF NOT EXISTS idx_users_part_role ON users(role, accountStatus)");
          } else if (tbl === 'withdrawals') {
            db.exec("CREATE INDEX IF NOT EXISTS idx_withdrawals_part_driver ON withdrawals(driverId, status)");
          } else if (tbl === 'bids') {
            db.exec("CREATE INDEX IF NOT EXISTS idx_bids_part_del ON bids(deliveryId, status)");
          }
          logDetails.push(`Index de partitionnement vérifiés pour la table '${tbl}'.`);
        } catch (idxErr: any) {
          logDetails.push(`Note index '${tbl}': ${idxErr.message}`);
        }
      });

      // 4. Déplacement / Partitionnement des anciennes données (selon rétention)
      try {
        const cutoffDate = new Date(Date.now() - retentionDays * 24 * 60 * 60 * 1000).toISOString();
        logDetails.push(`Seuil de partitionnement (avant le) : ${new Date(cutoffDate).toLocaleString('fr-FR')}`);
        
        // Partition tracking
        const trackingMoved = db.prepare(`
        ...
        if (trackingMoved.changes > 0) {
          db.prepare("DELETE FROM tracking WHERE timestamp < ?").run(cutoffDate);
          archivedCount += trackingMoved.changes;
          logDetails.push(`Partitionné & Archivé ${trackingMoved.changes} positions GPS historiques.`);
        }

        // Partition notifications lues
        const notifsMoved = db.prepare(`
        ...
        if (notifsMoved.changes > 0) {
          db.prepare("DELETE FROM notifications WHERE createdAt < ? AND isRead = 1").run(cutoffDate);
          archivedCount += notifsMoved.changes;
          logDetails.push(`Partitionné & Archivé ${notifsMoved.changes} notifications anciennes.`);
        }

        // Partition messages de chat anciens
        const chatMoved = db.prepare(`
        ...
        if (chatMoved.changes > 0) {
          db.prepare("DELETE FROM messages WHERE createdAt < ?").run(cutoffDate);
          archivedCount += chatMoved.changes;
          logDetails.push(`Partitionné & Archivé ${chatMoved.changes} messages de chat anciens.`);
        }
      ...
      // 5. Défragmentation & Optimisation Espace Disque
      if (!db.engine || db.engine.includes('SQLite')) {
        try {
          db.exec("PRAGMA wal_checkpoint(TRUNCATE)");
          db.exec("PRAGMA optimize");
          logDetails.push("Nettoyage WAL & Optimisation des requêtes effectués.");
          try {
            db.exec("VACUUM");
            logDetails.push("Reconstruction physique VACUUM effectuée.");
          } catch (vErr: any) {
            logDetails.push(`VACUUM léger ignoré : ${vErr.message}`);
          }
        } catch (optErr: any) {
      ...
      db.prepare("REPLACE INTO config (`key`, value) VALUES ('db_partition_status', ?)").run(JSON.stringify(resultStatus));

      res.json({
        success: true,
        message: "Partitionnement automatique et optimisation des tables terminés avec succès !",
      ...
      try {
        db.prepare("REPLACE INTO config (`key`, value) VALUES ('db_partition_status', ?)").run(JSON.stringify(failureStatus));
      } catch (_) {}

      res.status(500).json({
        success: false,
        error: `Échec du partitionnement: ${failureError}`,
        details: logDetails,
        errorMessage: failureError
      });
    }
  ...
  // Initial Seeding
  const seedConfig = () => {
    const hasConfig = db.prepare("SELECT `key`, value FROM config WHERE `key` = 'app_config'").get() as any;
    if (!hasConfig) {
      ...
      try {
        ...
        if (!current.appLogo || current.appLogo === '/logofaso.png' || current.appLogo === '/LOGOFASOEXPRESS_A.png' || current.appLogo.includes(' ')) {
          ...
          db.prepare("REPLACE INTO config (`key`, value) VALUES ('app_config', ?)").run(JSON.stringify(current));
          console.log("[MIGRATION] Logo mis à jour vers /LOGOFASO.png dans la configuration stockée.");
        }
      } catch (e) {
    ...
    for (const adminEmail of adminEmails) {
      try {
        ...
        if (!existingAdmin) {
          ...
          db.prepare("UPDATE users SET role = 'superadmin', accountStatus = 'active', userId = COALESCE(userId, id) WHERE email = ?")
            .run(adminEmail);
        }
      } catch (err) {
