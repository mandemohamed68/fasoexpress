...
const parseWeightNumber = (wVal: string, customWVal: string): number => {
  if (wVal === "custom") {
    if (!customWVal) return 0;
    const cleaned = customWVal.replace(/[^0-9.]/g, "");
    const parsed = parseFloat(cleaned);
    return isNaN(parsed) ? 0 : parsed;
  }
  const parsed = parseFloat(wVal);
  return isNaN(parsed) ? 0 : parsed;
};
...
export default function CreateDelivery() {
  ...
  useEffect(() => {
    const dup = location.state?.duplicateDelivery || location.state?.republishDelivery;
    if (dup) {
      const originLocation = dup.from || dup.origin;
      const destLocation = dup.to || dup.destination;
      if (originLocation && originLocation.lat && originLocation.lng) {
        setFrom({
          lat: Number(originLocation.lat),
          lng: Number(originLocation.lng),
          address: originLocation.address || "Point de départ"
        });
        setFromSearch(originLocation.address || "");
      }
      if (destLocation && destLocation.lat && destLocation.lng) {
        setTo({
          lat: Number(destLocation.lat),
          lng: Number(destLocation.lng),
          address: destLocation.address || "Destination"
        });
        setToSearch(destLocation.address || "");
      }
      if (dup.vehicleType) setVehicleType(dup.vehicleType);
      if (dup.packageDetails?.size) setSize(dup.packageDetails.size);
      if (dup.packageDetails?.weight) setWeight(dup.packageDetails.weight);
      if (dup.packageDetails?.notes || dup.notes) setNotes(dup.packageDetails?.notes || dup.notes || "");
      if (dup.senderPhone) setSenderPhone(dup.senderPhone);
      if (dup.recipientPhone) setRecipientPhone(dup.recipientPhone);
      if (dup.cost || dup.clientProposedPrice) {
        setProposedPrice(dup.clientProposedPrice || dup.cost);
      }
      toast.success("Course dupliquée ! Les adresses et détails ont été pré-remplis.");
    }
  }, [location.state]);
  ...
  useEffect(() => {
    const w = parseWeightNumber(weight, customWeight);
    if (w > 20) {
  ...
  }, [weight, customWeight, size]);

  // Pricing logic update
  useEffect(() => {
    if (from && to && commissionSettings) {
      const rawHaversine = calculateDistance(from.lat, from.lng, to.lat, to.lng);
      const haversineDist = isNaN(rawHaversine) || rawHaversine < 0 ? 0 : rawHaversine;
      setDistance(haversineDist);

      const calculatePrice = (distToUse: number) => {
        let basePrice = 0;
        const safeDist = isNaN(distToUse) || distToUse < 0 ? 0 : distToUse;
        const currentWeight = parseWeightNumber(weight, customWeight);
        ...
        if (matchedRule) {
          ...
          if (vehicleType === "moto") {
            const motoBase10 = Number(defaultPricing.motoBase10) || 1000;
            const motoBase15 = Number(defaultPricing.motoBase15) || 1500;
            const motoCostAfter15 = Number(defaultPricing.motoCostPerKmAfter15) || 150;
            if (safeDist <= 10) basePrice = motoBase10;
            else if (safeDist <= 15) basePrice = motoBase15;
            else basePrice = motoBase15 + Math.ceil(safeDist - 15) * motoCostAfter15;
          } else if (vehicleType === "tricycle") {
            basePrice = 3000 + (safeDist > 5 ? Math.ceil(safeDist - 5) * 250 : 0);
          } else if (vehicleType === "camion") {
            basePrice = 7500 + (safeDist > 5 ? Math.ceil(safeDist - 5) * 500 : 0);
          }
        }
        ...
        if (isUrgent) {
          basePrice += (Number(defaultPricing.urgenceCost) || 500);
        }

        const isMinPriceActive = commissionSettings?.enableMinPriceConstraint !== false;
        const minPriceFloor = isMinPriceActive ? (Number(commissionSettings?.minDeliveryCost) || 500) : 0;
        const computedBase = (basePrice || 0) - (Number(discount) || 0);
        const finalBase = isNaN(computedBase) ? minPriceFloor : Math.max(minPriceFloor, computedBase);
        const roundedPrice = Math.round(isNaN(finalBase) ? minPriceFloor : finalBase);

        setEstimatedCost(roundedPrice);
        setProposedPrice(roundedPrice);
      };
  ...
  }, [from, to, commissionSettings, weight, customWeight, size, discount, isUrgent, vehicleType, pricingRules, defaultPricing]);

  const handleCreate = async () => {
    if (!profile || !from || !to) return;

    const isMinPriceActive = commissionSettings?.enableMinPriceConstraint !== false;
    const minPriceFloor = isMinPriceActive ? (commissionSettings?.minDeliveryCost || 500) : 0;
    const finalProposedPrice = Number(proposedPrice);
    
    if (isNaN(finalProposedPrice) || (isMinPriceActive && finalProposedPrice < minPriceFloor)) {
      toast.error(`Le prix proposé ne peut pas être inférieur au tarif minimal de ${minPriceFloor} FCFA.`);
      return;
    }
    ...
    try {
      ...
      const safeBaseCost = isNaN(estimatedCost) || !estimatedCost ? minPriceFloor : estimatedCost;
      const safeProposed = isNaN(finalProposedPrice) || !finalProposedPrice ? safeBaseCost : finalProposedPrice;
      ...
      const newDelivery = await api.deliveries.create({
        ...
        clientProposedPrice: safeProposed,
        cost: safeProposed,
        isUrgent,
              ...
              <div>
                ...
                {!packageImage ? (
                  <div className="flex gap-3">
                    <label
                      htmlFor="packageImageInputCamera"
                      className={cn(
                        "flex-1 bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl py-3.5 px-2 flex flex-col items-center justify-center gap-1.5 cursor-pointer hover:bg-slate-100 hover:border-slate-300 transition-all text-slate-500 group",
                        isCompressingImage && "opacity-50 pointer-events-none"
                      )}
                    >
                      <Camera className="w-5 h-5 text-slate-400 group-hover:text-indigo-600 transition-colors" />
                      <span className="text-[10px] font-bold group-hover:text-slate-800 text-center uppercase tracking-wider">
                        Prendre une photo
                      </span>
                    </label>

                    <label
                      htmlFor="packageImageInputGallery"
                      className={cn(
                        "flex-1 bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl py-3.5 px-2 flex flex-col items-center justify-center gap-1.5 cursor-pointer hover:bg-slate-100 hover:border-slate-300 transition-all text-slate-500 group",
                        isCompressingImage && "opacity-50 pointer-events-none"
                      )}
                    >
                      <ImageIcon className="w-5 h-5 text-slate-400 group-hover:text-indigo-600 transition-colors" />
                      <span className="text-[10px] font-bold group-hover:text-slate-800 text-center uppercase tracking-wider">
                        Ouvrir la galerie
                      </span>
                    </label>
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden mb-8">
              <div className="bg-indigo-50 px-5 py-4 border-b border-indigo-100">
                ...
                <p className="text-3xl font-black text-indigo-900">
                  {isNaN(estimatedCost) || !estimatedCost ? (commissionSettings?.minDeliveryCost || 500) : estimatedCost} F
                </p>
              </div>
              <div className="p-5">
                ...
                <div className="flex gap-3">
                  <button
                    onClick={() => {
                      const minFloor = (commissionSettings?.enableMinPriceConstraint !== false) ? (commissionSettings?.minDeliveryCost || 500) : 0;
                      setProposedPrice((p) => {
                        const current = Number(p) || estimatedCost || minFloor;
                        return Math.max(minFloor, current - 100);
                      });
                    }}
                    className="w-12 h-12 bg-slate-100 rounded-xl font-black text-slate-600 hover:bg-slate-200 transition-colors"
                  >
                    -
                  </button>
                  <input
                    type="number"
                    value={proposedPrice === "" ? "" : isNaN(Number(proposedPrice)) ? "" : proposedPrice}
                    onChange={(e) => {
                      ...
                      if (val === "") {
                        ...
                        setProposedPrice(isNaN(num) ? "" : num);
                      }
                    }}
                    onBlur={() => {
                      const isMinActive = commissionSettings?.enableMinPriceConstraint !== false;
                      const minFloor = isMinActive ? (commissionSettings?.minDeliveryCost || 500) : 0;
                      const valNum = Number(proposedPrice);
                      if (isMinActive && (isNaN(valNum) || valNum < minFloor)) {
                        setProposedPrice(minFloor);
                        toast(`Prix ajusté au tarif minimum obligatoire (${minFloor} FCFA).`, { icon: 'ℹ️' });
                      }
                    }}
                  ...
                  <button
                    onClick={() =>
                      setProposedPrice((p) => {
                        const current = Number(p) || estimatedCost || 500;
                        return current + 100;
                      })
                    }
                    className="w-12 h-12 bg-slate-100 rounded-xl font-black text-slate-600 hover:bg-slate-200 transition-colors"
                ...
                {commissionSettings?.enableMinPriceConstraint !== false && (
                  <p className="text-[10px] font-bold text-orange-600 mt-2 text-center flex items-center justify-center gap-1">
                    <ShieldCheck className="w-3.5 h-3.5" />
                    Prix minimal obligatoire activé ({commissionSettings?.minDeliveryCost || 500} FCFA min)
                  </p>
                )}
              </div>
            </div>
