...
import { Truck, Package, MapPin, ArrowRight, UserCheck, User, ShieldCheck, Mail, Lock, Phone, ChevronRight, Globe, Zap, Camera, CheckSquare, Settings, Facebook, Plus, Info, Eye, EyeOff, MessageSquare } from 'lucide-react';
...
export default function LandingView() {
  ...
  const [logoUrl, setLogoUrl] = useState(() => appConfig?.appLogo || logoImg);
  const [logoError, setLogoError] = useState(false);

  useEffect(() => {
    if (appConfig?.appLogo) {
      setLogoUrl(appConfig.appLogo);
      setLogoError(false);
    }
  }, [appConfig?.appLogo]);
  ...
  const getLogoSrc = (url: string) => {
    ...
    const buster = appConfig?.updatedAt ? `?v=${encodeURIComponent(appConfig.updatedAt)}` : '';
    return `${url}${buster}`;
  };
  ...
  useEffect(() => {
    ...
    if (savedRemember === 'false') {
      ...
      if (savedRole === 'client' || savedRole === 'driver' || savedRole === 'admin') {
        setLoginRole(savedRole as UserRole);
      }
    }
  }, []);
    ...
    try {
      if (isRegistering) {
        ...
        if (rememberMe) {
          ...
          localStorage.setItem('saved_role', loginRole);
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-4 mb-12">
            <div className="w-28 h-28 rounded-2xl flex items-center justify-center bg-white shadow-xl shadow-orange-500/10 border-2 border-orange-100 overflow-hidden p-1">
              {!logoError ? (
                 <img src={getLogoSrc(logoUrl)} alt="FASO EXPRESS Logo" onError={() => setLogoError(true)} className="w-full h-full object-contain" />
              ) : (
            ...
            <div className="flex flex-col">
              <div className="flex items-baseline space-x-0.5">
                {appConfig?.appName ? (
                  <span className="text-3xl font-black text-slate-900 tracking-tighter italic uppercase leading-none">
                    {appConfig.appName}
                  </span>
                ) : (
                  <>
              ...
              <span className="text-[10px] font-black tracking-[0.4em] text-slate-400 uppercase mt-1">
                {appConfig?.appSlogan || "Plateforme Logistique"}
              </span>
            </div>
          </div>
        ...
        <div className="w-full max-w-md my-auto pb-10 mt-12 lg:mt-auto">
          <div className="text-center mb-8 flex flex-col items-center">
            ...
            <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest">
              {appConfig?.appSlogan ? `${appConfig.appSlogan} • ${appConfig.appName || "FASO EXPRESS"}` : `Plateforme de Livraison Professionnelle ${appConfig?.appName || "FASO EXPRESS"}`}
            </p>
          </div>
                ...
                <form onSubmit={handleConfirmResetPassword} className="space-y-4">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className="w-full bg-slate-50 border border-slate-100 rounded-xl pl-12 pr-12 py-3.5 text-sm font-bold text-slate-900 focus:border-orange-500 transition-all outline-none"
                    />
                    <button
                      type="button"
                  ...
                  <div className="space-y-1.5 pt-4 border-t border-slate-200">
                    ...
                    <div className="relative">
                      <input 
                        ...
                        onChange={e => setPassword(e.target.value)}
                        placeholder="Min. 6 caractères"
                        className="w-full bg-white border border-slate-200 rounded-xl px-4 pr-12 py-3 text-sm font-bold text-slate-900 focus:border-orange-500 transition-all outline-none"
                      />
                      <button
                        type="button"
                        onClick={() => setShowRegisterPassword(!showRegisterPassword)}
                  ...
                  <div className="flex items-start gap-4 pt-4 border-t border-slate-100">
                     ...
                     <div>
                       ...
                       <p className="text-[9px] font-medium text-slate-400 mt-1 leading-relaxed">
                         En cochant cette case, vous vous engagez à respecter les{" "}
                         <a href="https://fasoexpress.net/Conditions_Generales" target="_blank" rel="noopener noreferrer" className="text-orange-600 underline font-bold">
                           Conditions Générales
                         </a>{" "}
                         et la{" "}
                         <a href="https://fasoexpress.net/Politique_de_Confidentialite" target="_blank" rel="noopener noreferrer" className="text-orange-600 underline font-bold">
            ...
            <form onSubmit={handleAuth} className="space-y-4">
              ...
              <div className="flex items-center justify-between pt-1">
                <label className="flex items-center gap-2 cursor-pointer select-none">
                  ...
                  />
                  <span className="text-[11px] font-bold text-slate-600">Se souvenir de moi</span>
                </label>
                <button
                  type="button"
