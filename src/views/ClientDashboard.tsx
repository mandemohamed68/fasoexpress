...
import { Package, Clock, CheckCircle, Navigation, User, Home, Plus, ChevronRight, X, Copy, Share, Compass, HelpCircle, MessageCircle, MessageSquare, RotateCcw } from 'lucide-react';
...
export default function ClientDashboard() {
  const { profile, appConfig } = useAuth();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const jobs = await api.deliveries.list();
        setDeliveries((prev) => JSON.stringify(prev) === JSON.stringify(jobs) ? prev : jobs);
      } catch (err) {
    ...
    <div className="min-h-screen bg-transparent flex flex-col relative pb-20 selection:bg-indigo-500/10 selection:text-indigo-600">
      ...
      <div className="relative z-10 flex-1 flex flex-col p-4 sm:p-6 lg:p-8 max-w-lg xl:max-w-5xl mx-auto w-full">
        ...
        <header className="flex justify-between items-center pt-8 mb-8">
           <div>
              <p className="text-[11px] font-black uppercase tracking-widest text-slate-500 mb-0.5">{appConfig?.appName || "FASO EXPRESS"}</p>
                  ...
                  >
                      ...
                      <div className="relative z-10 flex flex-col">
                        ...
                        <div className="mt-auto">
                          {!activeDelivery.driverId ? (
                          ...
                          ) : !activeDelivery.isPaid && activeDelivery.paymentMethod !== 'cash' && activeDelivery.status !== 'cancelled' ? (
                            activeDelivery.paymentStatus === 'pending_approval' ? (
                              ...
                              <button 
                                ...
                                className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-600/20 active:scale-95 group flex items-center justify-center gap-2"
                              >
                                Payer maintenant ({activeDelivery.cost} FCFA) <ChevronRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                              </button>
                            )
                          ) : (
           ...
           <div className="space-y-3">
              {recentDeliveries.length === 0 ? (
                ...
                recentDeliveries.map(d => {
                  ...
                  const statusConfig = d.status === 'delivered' ? { label: 'Livré', badge: 'bg-emerald-50 text-emerald-600 border-emerald-100' } :
                                      d.status === 'cancelled' ? { label: 'Annulé', badge: 'bg-rose-50 text-rose-600 border-rose-100' } :
                                      d.status === 'pending' ? { label: 'En attente', badge: 'bg-amber-50 text-amber-600 border-amber-100' } :
                                      { label: 'En cours', badge: 'bg-indigo-50 text-indigo-600 border-indigo-100' };
                  return (
                    <motion.div 
                      key={d.id} 
                      ...
                      whileTap={{ scale: 0.99 }}
                      className="bg-white p-4 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 border border-slate-100 hover:border-indigo-100 transition-all cursor-pointer group shadow-sm"
                    >
                       <div className="flex items-center gap-3 min-w-0 flex-1">
                         <div className={cn(
                           ...
                           d.status === 'delivered' ? 'bg-emerald-50 text-emerald-500' : 
                           d.status === 'cancelled' ? 'bg-rose-50 text-rose-500' : 'bg-amber-50 text-amber-500'
                         )}>
                           {logo ? (
                             <img 
                               src={logo} 
                               ...
                               onError={(e) => {
                                 ...
                                 if (parent) {
                                   parent.classList.remove('bg-white', 'p-1');
                                   if (d.status === 'delivered') parent.classList.add('bg-emerald-50', 'text-emerald-500');
                                   else if (d.status === 'cancelled') parent.classList.add('bg-rose-50', 'text-rose-500');
                                   else parent.classList.add('bg-amber-50', 'text-amber-500');
                                 }
                               }}
                             />
                           ) : (
                             d.status === 'delivered' ? <CheckCircle className="w-5 h-5" /> : 
                             d.status === 'cancelled' ? <X className="w-5 h-5" /> : <Clock className="w-5 h-5" />
                           )}
                         </div>
                         <div className="flex-1 min-w-0">
                           <div className="flex items-center gap-2 mb-1 flex-wrap">
                             <span className={cn(
                               ...
                               {d.status === 'delivered' && <CheckCircle className="w-2.5 h-2.5" />}
                               {d.status === 'cancelled' && <X className="w-2.5 h-2.5" />}
                               {statusConfig.label}
                             </span>
                       ...
                       <div className="flex items-center justify-between sm:justify-end gap-3 shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-100">
                         ...
                         <div className="flex items-center gap-2">
                           <button
                             onClick={(e) => {
                               e.stopPropagation();
                               navigate('/client/new', { state: { duplicateDelivery: d } });
                             }}
                             className="px-2.5 py-1.5 bg-orange-50 hover:bg-orange-100 text-orange-600 rounded-xl text-[10px] font-black uppercase tracking-wider flex items-center gap-1.5 border border-orange-200 transition-all active:scale-95 shadow-sm"
                             title="Republier cette course sans tout ressaisir"
                           >
