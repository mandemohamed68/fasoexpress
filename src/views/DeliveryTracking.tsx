...
export default function DeliveryTracking() {
  ...
  const translateSize = (size?: string) => {
    if (size === 'small') return 'Petit';
    if (size === 'medium') return 'Moyen';
    if (size === 'large') return 'Grand';
    return size || '';
  };
      ...
      {delivery && (
        ...
        <div className="absolute bottom-0 left-0 right-0 h-[65%] z-50 bg-slate-50 rounded-t-[40px] shadow-[0_-15px_50px_rgba(0,0,0,0.06)] flex flex-col pb-[calc(8rem+env(safe-area-inset-bottom))] xl:pb-12 border-t border-white">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
                ...
                {/* Payment Action Banner on Tracking Page - Only appears when driver is assigned/accepted */}
                {profile?.role === 'client' && delivery.clientId === profile.userId && !!delivery.driverId && !delivery.isPaid && delivery.paymentMethod !== 'cash' && delivery.status !== 'cancelled' && (
                   <div className="bg-white rounded-3xl p-6 shadow-lg shadow-indigo-500/5 border-2 border-indigo-100 mb-6 relative overflow-hidden">
                      <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-50/60 rounded-bl-full pointer-events-none z-0" />
                      <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                         <div>
                            <div className="flex items-center gap-2 mb-1.5">
                               <span className="w-2.5 h-2.5 rounded-full bg-amber-500 animate-pulse" />
                               <p className="text-[10px] font-black uppercase tracking-[0.2em] text-amber-600">Paiement Requis</p>
                            </div>
                            <h3 className="text-base sm:text-lg font-black tracking-tight text-slate-900">
                               Livreur assigné ! Procédez au paiement
                            </h3>
                            <p className="text-xs text-slate-500 font-bold mt-1">
                               Montant de la course : <span className="font-black text-indigo-600 text-sm">{delivery.cost} FCFA</span>
                            </p>
                         </div>

                         {delivery.paymentStatus === 'pending_approval' ? (
                            <div className="px-5 py-3 bg-amber-50 border border-amber-200/80 rounded-2xl flex items-center gap-2 text-amber-700 text-xs font-black">
                               <Clock className="w-4 h-4 animate-spin text-amber-600" />
                               <span>Vérification du reçu en cours...</span>
                            </div>
                         ) : (
                            <button
                               onClick={() => setShowPaymentModal(true)}
                               className="w-full sm:w-auto px-6 py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-indigo-600/20 active:scale-95 transition-all flex items-center justify-center gap-2"
                            >
                               <span>Payer la course ({delivery.cost} FCFA)</span>
                ...
                <div className="bg-white rounded-3xl p-5 lg:p-6 shadow-[0_15px_40px_-10px_rgba(0,0,0,0.05)] border border-slate-100 mb-6 relative overflow-hidden">
                 ...
                 {/* Photos de la course : Colis, Récupération, Livraison */}
                 {(delivery.packageDetails?.imageUrl || delivery.pickupProofImage || delivery.deliveryProofImage || delivery.proofImage) && (
                    <div className="bg-white rounded-3xl p-5 border border-slate-100 shadow-sm space-y-4 mb-6 font-sans">
                       <div className="flex items-center justify-between">
                          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Photos de la course</p>
                          <span className="text-[10px] font-black text-indigo-600 bg-indigo-50 px-2.5 py-1 rounded-full">
                             {[delivery.packageDetails?.imageUrl, delivery.pickupProofImage, delivery.deliveryProofImage || delivery.proofImage].filter(Boolean).length} / 3 photo(s)
                          </span>
                       </div>

                       <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                          {/* 1. Photo du Colis */}
                          {delivery.packageDetails?.imageUrl ? (
                             <div className="bg-slate-50/80 rounded-2xl p-3 border border-slate-100 flex flex-col justify-between">
                                <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 mb-2 flex items-center gap-1.5">
                                   <Package className="w-3.5 h-3.5 text-indigo-600" /> Photo Colis
                                </p>
                                <div 
                                   className="rounded-xl overflow-hidden border border-slate-200 bg-white h-44 flex justify-center items-center cursor-pointer group relative"
                                   onClick={() => setPreviewModalImage(delivery.packageDetails?.imageUrl || null)}
                                >
                                   <img 
                                      src={getCleanProofImage(delivery.packageDetails.imageUrl)} 
                                      alt="Photo du colis" 
                                      className="max-w-full max-h-44 object-contain group-hover:scale-105 transition-transform"
                          ...
                          {/* 2. Photo Preuve de Récupération */}
                          {delivery.pickupProofImage ? (
                             <div className="bg-slate-50/80 rounded-2xl p-3 border border-slate-100 flex flex-col justify-between">
                                <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 mb-2 flex items-center gap-1.5">
                                   <Camera className="w-3.5 h-3.5 text-emerald-600" /> Preuve Récupération
                                </p>
                                <div 
                                   className="rounded-xl overflow-hidden border border-slate-200 bg-white h-44 flex justify-center items-center cursor-pointer group relative"
                                   onClick={() => setPreviewModalImage(getCleanProofImage(delivery.pickupProofImage))}
                                >
                                   <img 
                          ...
                          {(delivery.deliveryProofImage || (!delivery.pickupProofImage && delivery.proofImage)) ? (
                             <div className="bg-slate-50/80 rounded-2xl p-3 border border-slate-100 flex flex-col justify-between">
                                <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 mb-2 flex items-center gap-1.5">
                                   <Camera className="w-3.5 h-3.5 text-indigo-600" /> Preuve Livraison
                                </p>
                                <div 
                                   className="rounded-xl overflow-hidden border border-slate-200 bg-white h-44 flex justify-center items-center cursor-pointer group relative"
                                   onClick={() => setPreviewModalImage(getCleanProofImage(delivery.deliveryProofImage || delivery.proofImage))}
                                >
                                   <img 
                ...
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex flex-col gap-6">
                    <div className="flex items-center justify-between">
                       ...
                       <div className="flex items-center gap-2">
                          {delivery.paymentMethod && getPaymentLogo(delivery.paymentMethod) && (
                             <img 
                                src={getPaymentLogo(delivery.paymentMethod)!} 
                                alt={delivery.paymentMethod} 
                    ...
                    {delivery.packageDetails && (
                       <div className="bg-slate-50/80 rounded-2xl p-4 border border-slate-100 space-y-2">
                          <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Détails du Colis</p>
                          <div className="flex flex-wrap gap-2 text-xs font-bold text-slate-700">
                             {delivery.packageDetails.size && <span className="bg-white px-3 py-1.5 rounded-xl border border-slate-200">Taille : {translateSize(delivery.packageDetails.size)}</span>}
                             {delivery.packageDetails.weightStr && <span className="bg-white px-3 py-1.5 rounded-xl border border-slate-200">Poids : {delivery.packageDetails.weightStr} kg</span>}
                             {delivery.packageDetails.notes && <span className="bg-white px-3 py-1.5 rounded-xl border border-slate-200">Nature : {delivery.packageDetails.notes}</span>}
                          </div>
                       </div>
                    )}
        ...
        {previewModalImage && (
           <div className="fixed inset-0 z-[200] bg-black/90 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in" onClick={() => setPreviewModalImage(null)}>
              <div className="relative max-w-4xl max-h-[90vh] w-full flex items-center justify-center">
                 <button onClick={() => setPreviewModalImage(null)} className="absolute top-2 right-2 w-10 h-10 bg-white/20 hover:bg-white/40 text-white rounded-full flex items-center justify-center transition-all">
                    <X className="w-6 h-6" />
                 </button>
                 <img src={previewModalImage} alt="Aperçu grand format" className="max-w-full max-h-[85vh] object-contain rounded-2xl shadow-2xl" />
              </div>
           </div>
        )}
        </>
