...
const MapBoundsHandler = ({ drivers, deliveries }: LiveMapProps) => {
  ...
  useEffect(() => {
    ...
    const isWithinBurkina = (lat: number, lng: number) => {
      return lat >= 9.0 && lat <= 15.5 && lng >= -6.0 && lng <= 3.0;
    };

    drivers.forEach(d => {
      if (d.currentLocation?.lat && d.currentLocation?.lng && isWithinBurkina(d.currentLocation.lat, d.currentLocation.lng)) {
        points.push([d.currentLocation.lat, d.currentLocation.lng]);
      }
    });

    deliveries.forEach(d => {
      if (d.from?.lat && d.from?.lng && isWithinBurkina(d.from.lat, d.from.lng)) {
        points.push([d.from.lat, d.from.lng]);
      }
      if (d.to?.lat && d.to?.lng && isWithinBurkina(d.to.lat, d.to.lng)) {
...
const LiveMap: React.FC<LiveMapProps> = ({ drivers, deliveries }) => {
  // Default center (Ouagadougou, Burkina Faso) if no points
