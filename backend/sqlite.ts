...
try {
  ...
  // --- AUTOMATIC DEMO DATA SEEDING FOR NEW / EMPTY ENVIRONMENT ---
  try {
    const clientsCount = db.prepare("SELECT COUNT(*) as count FROM users WHERE role = 'client'").get().count;
    if (clientsCount === 0) {
      console.log("[SEED] No clients or deliveries found. Seeding realistic demo data...");

      // 1. Seed clients
      const demoClients = [
        { id: 'cli-1', userId: 'cli-1', name: 'Mamadou Ouédraogo', email: 'mamadou@example.com', role: 'client', accountStatus: 'active' },
      ...
      const insertUser = db.prepare("INSERT OR IGNORE INTO users (id, userId, name, email, role, accountStatus) VALUES (?, ?, ?, ?, ?, ?)");
      ...
      // 2. Seed drivers
      const demoDrivers = [
        { id: 'drv-1', userId: 'drv-1', name: 'Sékou Traoré (Zem 1)', email: 'sekou@example.com', role: 'driver', accountStatus: 'active', status: 'online', vehicleType: 'Moto' },
      ...
      const insertDriver = db.prepare("INSERT OR IGNORE INTO users (id, userId, name, email, role, accountStatus, status, vehicleType) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
      ...
      const demoDeliveries = [
        {
          id: 'del-1',
          clientId: 'cli-1',
          clientName: 'Mamadou Ouédraogo',
          driverId: 'drv-1',
          driverName: 'Sékou Traoré (Zem 1)',
          origin: JSON.stringify({ lat: 12.3714, lng: -1.5197, address: "Marché Central de Ouagadougou" }),
        ...
        },
        {
          ...
          destination: JSON.stringify({ lat: 12.3941, lng: -1.4921, address: "Somgandé" }),
          cost: 2500,
          status: 'pending',
          paymentStatus: 'pending',
      ...
      db.prepare("INSERT OR IGNORE INTO announcements (id, title, message, type, targetRole) VALUES (?, ?, ?, ?, ?)")
        .run('ann-1', 'Bienvenue sur la plateforme !', 'Faso Express est désormais active et opérationnelle.', 'info', 'all');

      // 5. Seed sectors
      db.prepare("INSERT OR IGNORE INTO sectors (id, name, city) VALUES (?, ?, ?)")
        .run('sec-1', 'Centre-ville', 'Ouagadougou');
      db.prepare("INSERT OR IGNORE INTO sectors (id, name, city) VALUES (?, ?, ?)")
        .run('sec-2', 'Dassasgho', 'Ouagadougou');
